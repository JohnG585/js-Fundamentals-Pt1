...
function weightedAverageMovieReview(rachel, roger, jake) {
  return ((rachel*.25)+(roger)+(jake*2.2))/3;
}
...
function areaOfRing(outerRadius, innerRadius) {
  var outerRadius = Math.PI*Math.pow(outerRadius,2);
  var innerRadius = Math.PI*Math.pow(innerRadius,2);
...
function stopAt(source, stop) {
  var character = source.indexOf(stop);
  var newString = source.substring(0, character);
